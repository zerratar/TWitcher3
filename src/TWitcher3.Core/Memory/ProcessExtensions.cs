...
namespace TWitcher3.Core.Memory
{
    using SizeT = UIntPtr;
    ...
    public static class ExtensionMethods
    {
        ...
        public static bool ReadBytes(this Process process, IntPtr addr, int count, out byte[] val)
        {
            ...
            SizeT read;
            val = null;
            var readProcessMemory = WinAPI.ReadProcessMemory(process.Handle, addr, bytes, (SizeT)bytes.Length, out read);
            if (!readProcessMemory || read != (SizeT)bytes.Length)
            {
                return false;
            }
        ...
        public static bool WriteInt(this Process process, IntPtr addr, int value)
        {
            return WriteBytes(process, addr, BitConverter.GetBytes(value));
        }
